...
// TODO(finance): route through transact(world, company, -amount, category, description) once the
// ledger lands; until then this adjusts Company::cash directly. Never overdraws.
bool pay(World& world, CompanyId company, Credits amount, std::string_view category,
         std::string_view description) {
    (void)category;
    (void)description;
    Company* c = world.companies.get(company);
    if (c == nullptr || amount < 0 || c->cash < amount) {
        return false;
    }
    c->cash -= amount;
    return true;
...
void pay_wages(World& world) {
    ...
    for (auto [id, m] : world.crew) {
        ...
        if (pay(world, ship->owner, due, "wages", std::format("wages: {}", m.name))) {
            p.paid += due;
            ++p.paid_count;
...
HireResult hire(const Content& content, World& world, ShipId ship_id, CrewId member_id) {
    ...
    if (!pay(world, ship->owner, bonus, "wages", std::format("signing bonus: {}", m->name))) {
...
FireResult fire(World& world, CrewId member_id) {
    ...
    const Credits owed = m->wages_owed;
    Credits settled = 0;
    if (owed > 0 && pay(world, ship->owner, owed, "wages", std::format("back pay: {}", m->name))) {
        settled = owed;
    }
    if (ship->owner == world.player) {
