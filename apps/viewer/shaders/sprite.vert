...
struct Sprite {
    ...
    vec4 style;           // x: minimum core radius [px]; y: shape; z: glow strength; w: unused
};
...
layout(location = 2) out vec4 v_style; // x: quad half-size in core radii; y: shape; z: glow
...
void main() {
    ...
    v_uv = corner * extent;
    v_color = s.color;
    v_style = vec4(extent, s.style.y, s.style.z, 0.0);
}
